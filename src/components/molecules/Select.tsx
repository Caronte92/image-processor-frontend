...
const ButtonSelect = styled.button<{
  ...
  &:hover {
    ...
    background-color: ${props => (props.$enabled ? props.$color.hover.background : props.$color.disabled.background)};
    border: ${({ $enabled, $color, $hideBorder }) =>
      $hideBorder ? 'transparent' : getBorder($enabled, $color.default.border, $color.disabled.border)};
  }
`;
      ...
      {isDropdownVisible && (
        <DropdownContainer>
          {props.options.map(option => (
            <OptionContainer key={option.value} data-value={option.value} onClick={props.onclickCallback}>
              <OptionWrapper>
                <Texts
                  type={'p'}
                  text={option.text}
